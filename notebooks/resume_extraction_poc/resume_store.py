...
class ResumeStorage:
    ...
    def __init__(
        ...
        Args:
            normalized_data: Dict of engineer_id -> normalized_content
            embeddings: Dict of engineer_id -> embedding vector
        ...
        for engineer_id in normalized_data.keys():
            if engineer_id not in embeddings:
                print(f"    ⚠️ Skipping {engineer_id}: no embedding found")
                continue
                
            record = ResumeRecord(
                engineer_id=engineer_id,
                normalized_content=normalized_data[engineer_id],
                embedding=embeddings[engineer_id],
    ...
    def save_sqlite(self, output_path: str):
        ...
        for record in self.records:
            ...
            cursor.execute('''
                ...
                (engineer_id, normalized_content, embedding, embedding_model, embedding_dim, processed_at)
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (
                record.engineer_id,
