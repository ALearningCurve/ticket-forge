...
class TestHelpers:
  def test_optional_timestamptz_filters_invalid_values(self) -> None:
    ...
    assert load_tickets._optional_timestamptz("2026-01-01T00:00:00Z") == "2026-01-01T00:00:00Z"
...
class TestUpserts:
  def test_upsert_tickets_sanitizes_hours_and_timestamps(self) -> None:
    ...
    with patch("training.etl.postload.load_tickets.psycopg2.connect", return_value=conn):
  ...
  def test_upsert_assignments_handles_missing_and_unassigned(self) -> None:
    ...
    with patch("training.etl.postload.load_tickets.psycopg2.connect", return_value=conn):
      upserted, missing = load_tickets.upsert_assignments(tickets, dsn="postgresql://fake")
...
class TestPipeline:
  def test_run_pipeline_orchestrates_steps(self) -> None:
    ...
    with patch("training.etl.postload.load_tickets.scrape_all_issues", _fake_scrape):
      with patch("training.etl.postload.load_tickets.transform_records", _fake_transform):
        with patch("training.etl.postload.load_tickets.upsert_tickets", _fake_upsert_tickets):
          with patch(
