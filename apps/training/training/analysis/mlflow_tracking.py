...
def _log_model_run(
  ...
  with mlflow.start_run(
    ...
    # Log sensitivity analysis plots (generated by run_sensitivity_analysis
    # before log_run_to_mlflow is called, so files exist at this point).
    for plot_name in (
    ...
    if pkl_path.exists():
      try:
        ...
        mlflow.log_params(
          {k: str(v) for k, v in (grid.best_params_ or {}).items()}
        )
      except Exception:
        logger.warning(
    ...
    # Log individual trials as deeper nested runs under this model run.
...
def log_run_to_mlflow(run_id: str) -> str | None:
  ...
  try:
    ...
    logger.info(
      "MLflow logging complete — parent run ID: %s", parent_run_mlflow_id
    )
...
def promote_best_model(run_id: str) -> str | None:
  ...
  (filesystem backend does not support the registry). Set MLFLOW_TRACKING_URI
  to a database-backed URI (e.g. sqlite:///mlflow.db or a remote server) when
  ...
  with open(best_file) as f:
    for line in f:
      if line.startswith("Best Model:"):
        best_model_name = line.replace("Best Model:", "").strip()
        break
  ...
  # Register model — mlflow creates a new version automatically
  try:
  ...
  # Fetch the latest version that was just created
  try:
    versions = client.get_latest_versions(
      _REGISTERED_MODEL_NAME, stages=["None"]
    )
    if not versions:
      logger.warning(
    ...
    logger.exception(
      "Could not fetch latest version of '%s'", _REGISTERED_MODEL_NAME
    )
    return None

  # Archive existing Production versions, then promote the new one
  try:
    all_versions = client.search_model_versions(
      f"name='{_REGISTERED_MODEL_NAME}'"
    )
    for mv in all_versions:
      if mv.current_stage == "Production" and mv.version != str(new_version):
