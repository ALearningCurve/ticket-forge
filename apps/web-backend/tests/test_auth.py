...
class TestSignup:
  ...
  async def test_signup_weak_password_no_uppercase(
    self, client: AsyncClient
  ) -> None:
  ...
  async def test_signup_weak_password_no_digit(
    self, client: AsyncClient
  ) -> None:
  ...
  async def test_signup_weak_password_too_short(
    self, client: AsyncClient
  ) -> None:
  ...
  async def test_signup_missing_fields(
    ...
    resp = await client.post(
      "/api/v1/auth/signup", json={"username": "test"}
    )
    assert resp.status_code == 422
...
class TestSignin:
  ...
  async def test_signin_username_case_insensitive(
    self, client: AsyncClient
  ) -> None:
...
class TestFullFlow:
  ...
  async def test_complete_auth_lifecycle(
    self, client: AsyncClient
  ) -> None:
